...
function forwardmodel(soc0,s_realized)
    ...
    @variable(m, 0 <= soc[rtm] <= 100)		                #SOC of the battery at the end of each real time interval      
    ...
    @variable(m, theta >= 0)
    @constraint(m, InitialEnergy, ebat[1] == soc0/100*ebat_max - 1/eff*Pnet[1]*dtrtm - suppliedload[1]*dtrtm)	#Inital energy in the battery
    
    @constraint(m, DefSOC[i in rtm], soc[i] == ebat[i]/ebat_max*100)			#Define SOC
   ...
   # @constraint(m, DAMRamp[i=rtm[1],k in dam[2:end],iend=rtm[end],s in S], rampmin*dtrtm <= Pnet[i,k,s] - Pnet[iend,k-1,s] <= rampmax*dtrtm)   #Ramp discharge constraint at each time    
    ...
    @constraint(m, UnmetLoad[i in rtm], suppliedload[i] + unmetload[i] >=  load[i,s_realized][1])

    @constraint(m, BoundSupplied[i in rtm], suppliedload[i] <= load[i,s_realized][1])

    @constraint(m, BoundUnmet[i in rtm], unmetload[i] <= load[i,s_realized][1])
...
function scenariosubproblem(soc0,s,dual,obj)
...
for p in 1:nhours_planning-1
    ...
    s_realized = rand(S,1);
    m_f[p] = forwardmodel(soc0,s_realized);
    status = solve(m_f[p]);
    soc0 = getvalue(getvariable(m_f[p],:soc))[rtm[end]];
...
for p in nhours_planning:-1:2
    soc0_b = getvalue(getvariable(m_f[p-1],:soc))[end];
    for s in S
        println("Hour $p, Scenario $s")
        m_b[s,p] = scenariosubproblem(soc0_b,s);
        status = solve(m_b);
        dual[s] = getdual(getconstraint(m_b[s,p],:rtmEBalance))[rtm[end]];
        obj[s] = getobjectivevalue(m_b[s,p]);
        v[s] = obj[s] - dual[s]*soc0_b;
    end
