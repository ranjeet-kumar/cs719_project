...
################ Model ##################
tic()
m_sf = Model(solver = GurobiSolver(Threads=2))
		@variable(m_sf, -P_max <= Prtm[rtm,dam,day,S] <= P_max)	                #Power sold to the real time market, kW
		@variable(m_sf, -P_max <= Pdam[dam,day,S] <= P_max)    	                #Power sold to the day ahead market, kW
		@variable(m_sf, 0 <= ebat[rtm,dam,day,S] <= ebat_max)      	#Energy stored in the battery at the end of each real time interval, kWh
		@variable(m_sf, suppliedload[rtm,dam,day,S] >= 0)
		@variable(m_sf, unmetload[rtm,dam,day,S] >= 0)
		@expression(m_sf, Pnet[i in rtm,k in dam,l in day,s in S], Prtm[i,k,l,s] + Pdam[k,l,s] + suppliedload[i,k,l,s])    #Net power discharged from battery in all 5-min interval, kW
		@variable(m_sf, profitErtm[rtm,dam,day,S])# >= 0)				        #Profit from the real time market, USD
		...
		@constraint(m_sf, UnmetLoad[i in rtm,k in dam,l in day, s in S], suppliedload[i,k,l,s] + unmetload[i,k,l,s] >=  loadperm[i,k,l,s])
		@constraint(m_sf, BoundSupplied[i in rtm,k in dam,l in day,s in S], suppliedload[i,k,l,s] <= loadperm[i,k,l,s])
		@constraint(m_sf, BoundUnmet[i in rtm,k in dam,l in day,s in S], unmetload[i,k,l,s] <= loadperm[i,k,l,s])
    @constraint(m_sf, NetDischarge1[i in rtm,k in dam,l in day,s in S], Pnet[i,k,l,s] <= P_max)
    @constraint(m_sf, NetDischarge2[i in rtm,k in dam,l in day,s in S], Pnet[i,k,l,s] >= -P_max)
    #=    @constraint(m_sf, RTMRamp1[i in rtm[2:end],k in dam,l in day,s in S], Pnet[i,k,l,s]  - Pnet[i-1,k,l,s] <= rampmax*dtrtm)   #Ramp discharge constraint at each time
		@constraint(m_sf, RTMRamp2[i in rtm[2:end],k in dam,l in day,s in S], Pnet[i,k,l,s]  - Pnet[i-1,k,l,s] >= -rampmax*dtrtm)   #Ramp discharge constraint at each time
		@constraint(m_sf, DAMRamp1[i in rtm[1],k in dam[2:end],iend=rtm[end],l in day,s in S], Pnet[i,k,l,s] - Pnet[iend,k-1,l,s] <= rampmax*dtrtm)   #Ramp discharge constraint at each time
		@constraint(m_sf, DAMRamp2[i in rtm[1],k in dam[2:end],iend=rtm[end],l in day,s in S], Pnet[i,k,l,s] - Pnet[iend,k-1,l,s] >= -rampmax*dtrtm)   #Ramp discharge constraint at each time
		@constraint(m_sf, DAYRamp1[i=rtm[1],k=dam[1],iend=rtm[end],kend=dam[end],l in day[2:end],s in S], Pnet[i,k,l,s] - Pnet[iend,kend,l-1,s] <= rampmax*dtrtm)   #Ramp discharge constraint at each time
		@constraint(m_sf, DAYRamp2[i=rtm[1],k=dam[1],iend=rtm[end],kend=dam[end],l in day[2:end],s in S], Pnet[i,k,l,s] - Pnet[iend,kend,l-1,s] >= -rampmax*dtrtm)   #Ramp discharge constraint at each time
		=#
		@constraint(m_sf, RTMEProfits[i in rtm,k in dam,l in day,s in S], profitErtm[i,k,l,s] == rtmepr[i,k,l]*Prtm[i,k,l,s]*dtrtm)	#Economic calculation
		@constraint(m_sf, DAMEProfits[k in dam,l in day,s in S], profitEdam[k,l,s] == damepr[k,l]*Pdam[k,l,s]*dtdam)	#Economic calculation
		@constraint(m_sf, TotalProfit[s in S], profittotal[s] ==
												sum{profitErtm[i,k,l,s], i in rtm, k in dam, l in day} + sum{profitEdam[k,l,s], k in dam, l in day})
		@constraint(m_sf, UnmetCost[s in S], unmetcost[s] == sum{rtmepr[i,k,l]*unmetload[i,k,l,s], i in rtm, k in dam, l in day})
		# Non-anticipativity constraints for first stage variables
		@constraint(m_sf, Nonant_PDAM[k in dam,l in day,s in S], Pdam[k,l,s] == (1/NS)*sum{Pdam[k,l,s], s in S})
		@constraint(m_sf, Nonant_EbatHourEnd[i in rtm[end], k in dam,l in day,s in S], ebat[i,k,l,s] == (1/NS)*sum{ebat[i,k,l,s], s in S})
		@objective(m_sf, Min, (1/NS)*sum{-profittotal[s] + unmetcost[s], s in S})
		if participate_dam == 0
      @constraint(m_sf, NoDAM[k in dam,l in day, s in S], Pdam[k,l,s] == 0)
    end
    if participate_rtm == 0
      @constraint(m_sf, NoRTM[i in rtm,k in dam,l in day, s in S], Prtm[i,k,l,s] == 0)
    end
		status = solve(m_sf)
time_taken_st_fp_socfix = toc();
obj_st_fp_socfix = getobjectivevalue(m_sf);
...
tick_params(labelsize=14)
#legend(loc="upper right",fancybox="True", shadow="True", fontsize = 15)
savefig(string("cs719figures/Pdam_fp_st_socfix.pdf"))
close("all")
...
Prtmarray = convertToArray4(getvalue(getvariable(m_sf,:Prtm)),n4);
...
tick_params(labelsize=14)
#legend(loc="upper right",fancybox="True", shadow="True", fontsize = 15)
savefig(string("cs719figures/Prtm_fp_st_socfix.pdf"))
close("all")

# Plot of supplied load and unmet load
n4 = [nrtm,ndam,ndays_planning,NS];
suppliedloadarray = convertToArray4(getvalue(getvariable(m_sf,:suppliedload)),n4);
suppliedloadplot = reshape(suppliedloadarray,nrtm_planning,NS);
suppliedloadplot = [suppliedloadplot;suppliedloadplot[end,:]];
unmetloadarray = convertToArray4(getvalue(getvariable(m_sf,:unmetload)),n4);
...
tick_params(labelsize=14)
#legend(loc="upper right",fancybox="True", shadow="True", fontsize = 15)
savefig(string("cs719figures/supp_unmet_fp_st_socfix.pdf"))
close("all")


# Plot of SOC
n4 = [nrtm,ndam,ndays_planning,NS];
socarray = convertToArray4(getvalue(getvariable(m_sf,:ebat)),n4)/ebat_max*100;
...
tick_params(labelsize=14)
#legend(loc="upper right",fancybox="True", shadow="True", fontsize = 15)
savefig(string("cs719figures/soc_fp_st_socfix.pdf"))
close("all")
...
tick_params(labelsize=14)
#legend(loc="upper right",fancybox="True", shadow="True", fontsize = 15)
savefig(string("cs719figures/netpower_fp_st_socfix.pdf"))
close("all")

# Plot of Profits from Pdam and Prtm and Total
n3 = [ndam,ndays_planning,NS];
profitEdamarray = convertToArray3(getvalue(getvariable(m_sf,:profitEdam)),n3);
...
n4 = [nrtm,ndam,ndays_planning,NS];
profitErtmarray = convertToArray4(getvalue(getvariable(m_sf,:profitErtm)),n4);
...
savefig(string("cs719figures/cumulative_rev_fp_st_socfix.pdf"))
close("all")
end
