...
function forwardmodel(ebat0,s_realized)
...
for p in 1:nhours_planning
    ...
    s_realized = rand(S);
    m_f[p] = forwardmodel(ebat0,s_realized);
    status = solve(m_f[p]);
    ebat0 = getvalue(getvariable(m_f[p],:ebat))[rtm[end]];
end # End forward pass
lowerbound = getobjectivevalue(m_f[1]);
...
s_realized = rand(S);
for i in rtm
  JuMP.setRHS(getconstraint(m_f[p],:UnmetLoad)[i],load[i,s_realized][1])
  JuMP.setRHS(getconstraint(m_f[p],:BoundSupplied)[i],load[i,s_realized][1])
  JuMP.setRHS(getconstraint(m_f[p],:BoundUnmet)[i],load[i,s_realized][1])
end
theta = getvariable(m_f[p],:theta)
ebat = getvariable(m_f[p],:ebat)
@constraint(m_f[p], ThetaConst, theta >= v_avg + pi_avg*ebat[rtm[end]])
status = solve(m_f[p]);
ebat0 = getvalue(getvariable(m_f[p],:ebat))[rtm[end]];

for p in 2:nhours_planning
    println("Step $p")
    ...
    s_realized = rand(S);
    m_f[p] = m_b[s_realized,p];
    JuMP.setRHS(getconstraint(m_f[p],:InitialEnergy), ebat0)
    status = solve(m_f[p]);
    ebat0 = getvalue(getvariable(m_f[p],:ebat))[rtm[end]];
